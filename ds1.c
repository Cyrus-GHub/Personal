...
int deleteEvenNumbers(int *arr2D, int arr1D[], int row, int col)
{
    ...
    for (int i = 0; i < row; i++)
    {
        for (int j = 0; j < col; j++)
        {
            if (*(arr2D + i * col + j) % 2 == 0)
                continue;

            arr1D[idx] = *(arr2D + i * col + j);
            idx++;
        }
    }
}
...
int convert_1D_Column_Wise_to_2D(int arr1D[], int *arr2D, int row, int col)
{
    int n = 0;
    for (int i = 0; i < row; i++)
    {
...
int main()
{
    ...
    */
    ...
    /*
    Q4: A 1D Matrix ordered in Column-major is converted to 2D Matrix.

    (Making this function simple by taking the rows and columns of the 2D matrix.
        As 1D Matrix of size 12 can have many forms of 2D matrix, like:
        1x12, 2x6, 3x4, 4x3, 6x2, 12x1
    )
    */

    int recreate2D[ROW][COL];
    printf("\n\n1D Column-Wise Matrix to 2D\n");
    printf("----------\n");
    convert_1D_Column_Wise_to_2D(arr1D, (int *)recreate2D, ROW, COL);
    display2DMatrix((int *)arr2D, ROW, COL);
